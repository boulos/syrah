#ifndef _SYRAH_FIXED_VECTOR_AVX_INT_H_
#define _SYRAH_FIXED_VECTOR_AVX_INT_H_
...
for (int index = 0; index < N/4; index++)
    ...
    SYRAH_FORCEINLINE explicit FixedVector(const FixedVector<float, N, true>& v) {
      SYRAH_SSE_LOOP(i) {
        data[i] = _mm_cvttps_epi32(_mm256_extractf128_ps(v.data[i/2], i & 1));
      }
    }
    ...
    SYRAH_FORCEINLINE void load(const int* values, const FixedVectorMask<N>& mask,
      ...
      if (sse_aligned) {
        SYRAH_SSE_LOOP(i) {
          ...
          __m128i int_mask = _mm_castps_si128(_mm256_extractf128_ps(mask.data[i/2], i&1));
          data[i] = syrah_blendv_int(loaded_data, default_data, int_mask);
        }
      } else {
        // TODO(boulos): If N is long enough, this could probably be
        // improved.
    ...
    SYRAH_FORCEINLINE void store(int* dst, const FixedVectorMask<N>& mask) const {
      ...
      if (sse_aligned) {
        SYRAH_SSE_LOOP(i) {
          ...
          __m128i int_mask = _mm_castps_si128(_mm256_extractf128_ps(mask.data[i/2], i&1));
          _mm_store_si128((__m128i*)&(dst[4*i]), syrah_blendv_int(data[i], cur_val, int_mask));
        }
      } else {
        SYRAH_SSE_LOOP(i) {
          __m128i cur_val = _mm_loadu_si128((__m128i*)&(dst[4*i]));
          __m128i int_mask = _mm_castps_si128(_mm256_extractf128_ps(mask.data[i/2], i&1));
          _mm_storeu_si128((__m128i*)&(dst[4*i]), syrah_blendv_int(data[i], cur_val, int_mask));
        }
      }
    }
    ...
    SYRAH_FORCEINLINE void merge(const FixedVector<int, N, true>& v,
      ...
      SYRAH_SSE_LOOP(i) {
        __m128i int_mask = _mm_castps_si128(_mm256_extractf128_ps(mask.data[i/2], i&1));
        data[i] = syrah_blendv_int(v.data[i], data[i], int_mask);
      }
    }
    ...
    SYRAH_FORCEINLINE FixedVectorMask<N, true> operator <(const FixedVector<int, N, true>& v) const {
      ...
      SYRAH_SSE_LOOP(i) {
        result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_cmplt_epi32(data[i], v.data[i])), i & 1);
      }
      return result;
    }
    ...
    SYRAH_FORCEINLINE FixedVectorMask<N, true> operator <=(const FixedVector<int, N, true>& v) const {
      FixedVectorMask<N, true> result;
      SYRAH_SSE_LOOP(i) {
        result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_or_si128(_mm_cmplt_epi32(data[i], v.data[i]),
                                                                                                _mm_cmpeq_epi32(data[i], v.data[i]))), i & 1);
      }
      return result;
    }
    ...
    SYRAH_FORCEINLINE FixedVectorMask<N, true> operator ==(const FixedVector<int, N, true>& v) const {
      FixedVectorMask<N, true> result;
      SYRAH_SSE_LOOP(i) {
        result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_cmpeq_epi32(data[i], v.data[i])), i&1);
      }
      return result;
    }
    ...
    SYRAH_FORCEINLINE FixedVectorMask<N, true> operator >=(const FixedVector<int, N, true>& v) const {
      FixedVectorMask<N, true> result;
      SYRAH_SSE_LOOP(i) {
        result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_or_si128(_mm_cmpgt_epi32(data[i], v.data[i]),
                                                                                                _mm_cmpeq_epi32(data[i], v.data[i]))), i&1);
      }
      return result;
    }
    ...
    SYRAH_FORCEINLINE FixedVectorMask<N, true> operator >(const FixedVector<int, N, true>& v) const {
      FixedVectorMask<N, true> result;
      SYRAH_SSE_LOOP(i) {
        result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_cmpgt_epi32(data[i], v.data[i])), i&1);
      }
      return result;
    }
  ...
  SYRAH_FORCEINLINE FixedVectorMask<N, true> operator<(const int& s, const FixedVector<int, N, true>& v) {
    ...
    const __m128i splat = _mm_set1_epi32(s);
    SYRAH_SSE_LOOP(i) {
      result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_cmplt_epi32(splat, v.data[i])), i & 1);
    }
    return result;
  }
  ...
  SYRAH_FORCEINLINE FixedVectorMask<N, true> operator<(const FixedVector<int, N, true>& v, const int& s) {
    ...
    const __m128i splat = _mm_set1_epi32(s);
    SYRAH_SSE_LOOP(i) {
      result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_cmplt_epi32(v.data[i], splat)), i & 1);
    }
    return result;
  }
  ...
  SYRAH_FORCEINLINE FixedVectorMask<N, true> operator<=(const int& s, const FixedVector<int, N, true>& v) {
    ...
    const __m128i splat = _mm_set1_epi32(s);
    SYRAH_SSE_LOOP(i) {
      result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_or_si128(_mm_cmplt_epi32(splat, v.data[i]),
                                                                                              _mm_cmpeq_epi32(splat, v.data[i]))), i & 1);
    }
    return result;
  }
  ...
  SYRAH_FORCEINLINE FixedVectorMask<N, true> operator<=(const FixedVector<int, N, true>& v, const int& s) {
    ...
    const __m128i splat = _mm_set1_epi32(s);
    SYRAH_SSE_LOOP(i) {
      result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_or_si128(_mm_cmplt_epi32(v.data[i], splat),
                                                                                              _mm_cmpeq_epi32(v.data[i], splat))), i & 1);
    }
    return result;
  }
  ...
  SYRAH_FORCEINLINE FixedVectorMask<N, true> operator==(const FixedVector<int, N, true>& v, const int& s) {
    ...
    const __m128i splat = _mm_set1_epi32(s);
    SYRAH_SSE_LOOP(i) {
      result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_cmpeq_epi32(v.data[i], splat)), i & 1);
    }
    return result;
  }
  ...
  SYRAH_FORCEINLINE FixedVectorMask<N, true> operator==(const int& s, const FixedVector<int, N, true>& v) {
    ...
    const __m128i splat = _mm_set1_epi32(s);
    SYRAH_SSE_LOOP(i) {
      result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_cmpeq_epi32(splat, v.data[i])), i & 1);
    }
    return result;
  }
  ...
  SYRAH_FORCEINLINE FixedVectorMask<N, true> operator>=(const int& s, const FixedVector<int, N, true>& v) {
    ...
    const __m128i splat = _mm_set1_epi32(s);
    SYRAH_SSE_LOOP(i) {
      result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_or_si128(_mm_cmpeq_epi32(splat, v.data[i]),
                                                                                              _mm_cmplt_epi32(v.data[i], splat))), i & 1);
    }
    return result;
  }
  ...
  SYRAH_FORCEINLINE FixedVectorMask<N, true> operator>=(const FixedVector<int, N, true>& v, const int& s) {
    ...
    const __m128i splat = _mm_set1_epi32(s);
    SYRAH_SSE_LOOP(i) {
      result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_or_si128(_mm_cmpeq_epi32(v.data[i], splat),
                                                                                              _mm_cmplt_epi32(splat, v.data[i]))), i & 1);
    }
    return result;
  }
  ...
  SYRAH_FORCEINLINE FixedVectorMask<N, true> operator !=(const int& s, const FixedVector<int, N, true>& v) {
    ...
    SYRAH_SSE_LOOP(i) {
      // NOT= & TRUE => !=
      result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_andnot_si128(_mm_cmpeq_epi32(splat, v.data[i]), all_ones)), i & 1);
    }
    return result;
  }
  ...
  SYRAH_FORCEINLINE FixedVectorMask<N, true> operator !=(const FixedVector<int, N, true>& v, const int& s) {
    ...
    SYRAH_SSE_LOOP(i) {
      // NOT= & TRUE => !=
      result.data[i/2] = _mm256_insertf128_ps(result.data[i/2], _mm_castsi128_ps(_mm_andnot_si128(_mm_cmpeq_epi32(v.data[i], splat), all_ones)), i & 1);
    }
    return result;
  }
  ...
  SYRAH_FORCEINLINE FixedVector<int, N, true> select(const FixedVector<int, N, true>& a,
    ...
    SYRAH_SSE_LOOP(i) {
      __m128i int_mask = _mm_castps_si128(_mm256_extractf128_ps(mask.data[i/2], i & 1));
      result.data[i] = syrah_blendv_int(a.data[i], b.data[i], int_mask);
    }
    return result;
  }
...
#endif // _SYRAH_FIXED_VECTOR_AVX_INT_H_
