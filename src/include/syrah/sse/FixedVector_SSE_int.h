...
for (int index = 0; index < N/4; index++)
  ...
  template<int N>
  SYRAH_FORCEINLINE FixedVector<int, N, true> splat(const FixedVector<int, N, true>& a, int which) {
    FixedVector<int, N, true> result;
    ...
    switch (which_elem) {
    case 0: SYRAH_SSE_LOOP(i) { result.data[i] = _mm_shuffle_epi32(a.data[which_sse], _MM_SHUFFLE(0, 0, 0, 0)); } break;
    case 1: SYRAH_SSE_LOOP(i) { result.data[i] = _mm_shuffle_epi32(a.data[which_sse], _MM_SHUFFLE(1, 1, 1, 1)); } break;
    case 2: SYRAH_SSE_LOOP(i) { result.data[i] = _mm_shuffle_epi32(a.data[which_sse], _MM_SHUFFLE(2, 2, 2, 2)); } break;
    default: SYRAH_SSE_LOOP(i) { result.data[i] = _mm_shuffle_epi32(a.data[which_sse], _MM_SHUFFLE(3, 3, 3, 3)); } break;
    }
    return result;
  }
